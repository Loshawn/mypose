...
class WholebodyEval:
    ...
    def __init__(self,
        ...
        self.kpt_name = position \
            if position == "keypoints" or position == "wholebody" else f"{position}_kpts"
    ...
    def _prepare(self):
        ...
        for gt in gts:
            g = np.array(
                gt.get(
                    self.kpt_name,
                    gt["keypoints"] + gt["foot_kpts"] + gt["face_kpts"] +
                    gt["lefthand_kpts"] + gt["righthand_kpts"]))
    ...
    def evaluate(self):
        ...
        if not p.useSegm is None:
            ...
            print('useSegm (deprecated) is not None. Running {} evaluation'.
                  format(p.iouType))
        print(f'Evaluate annotation type *{p.iouType} ({self.position})*')
    ...
    def computeOks(self, imgId, catId):
        ...
        wholebody = keypoints + foot + face + lefthand + righthand
        ...
        sigmas = np.array(all_sigmas[self.position])
