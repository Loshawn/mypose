...
class Eval_righthand:
    ...
    def computeIoU(self, imgId, catId):
        ...
        if len(gt) == 0 and len(dt) == 0:
            return []
        inds = np.argsort([-d['righthand_score'] for d in dt],
                          kind='mergesort')
    ...
    def computeOks(self, imgId, catId):
        ...
        inds = np.argsort([-d['righthand_score'] for d in dts],
                          kind='mergesort')
        ...
        sigmas = np.array([
            0.29, 0.22, 0.35, 0.37, 0.47, 0.26, 0.25, 0.24, 0.35, 0.18, 0.24,
            0.22, 0.26, 0.17, 0.21, 0.21, 0.32, 0.2, 0.19, 0.22, 0.31
        ]) / 10.0
        vars = (sigmas * 2)**2
        k = len(sigmas)
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet):
        ...
        dtind = np.argsort([-d['righthand_score'] for d in dt],
                           kind='mergesort')
